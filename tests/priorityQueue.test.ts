...
describe('PriorityQueue', () => {
  ...
  test('updatePriority changes priority', () => {
    const queue = new PriorityQueue();
    queue.enqueue("lane1", 2, 0);
    queue.enqueue("lane2", 3, 0);

    queue.updatePriority("lane2", 1); // Increase priority (lower number)
  ...
  test('active items have correct status', () => {
    ...
    const item = queue.get("lane1");
    expect(item).toBeDefined();
    expect(item?.activeSince).toBe(undefined);
